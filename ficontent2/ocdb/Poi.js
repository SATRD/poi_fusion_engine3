...
var getResource = function(ep,params,query,body,cb,uid){
	...
	if(params.cityid && ep.type==="collection"){
		if(typeof query.lon2!="undefined" && typeof query.lat2!="undefined" ){
			...
			var q = {search: query.search||'', limit: (parseInt(query.limit) || 20), offset: (parseInt(query.offset) || 0), uid:uid};
			if (query.tags) q.tags = query.tags; //UPV ADDED 			
...
var postResource = function(ep,params,query,body,cb,uid){	
				...
				if(!e){
						...
						if(err && !changesElements) {
							...
							if (mediaTmp && mediaTmp._media && mediaTmp._media.length) {
								var mediaObjects = _.map(mediaTmp._media, function(v) {v.user={_user:uid}; var m = new Media(v); m.save(); return m; } );
	...
	if(params.poiid && ep.type==="instance"){
		...
		//update existing poi
		if(ep.path.indexOf("updatePoiAttributes")!=-1){
			...
			Poi.update({_id: params.poiid},{'attributes':  body.attributes },
					{upsert: false},
					function(e,r){
						if(!e && r) {
							console.log("updated poi attributes");	
						} else {
							console.log(e);						
						}
					}
		...
		input.user = {
  		...
  		var poiFoundCB = function(poi){
			...
			if(ep.path.indexOf("checkin")>=0){
				// UPV ADDED Check if the user has already chek in that point/POI. He /she can only chek once (for Fallas)
				
				var q = {coords: input.coords,user:input.user} ;
		
				Checkin.find(q).exec(function(e,res){
					if (e){
						...
						if (res.length > 0){
							console.log("user already made chekin here");
							cb(500,{});
							return;
						}else{
							//input: -
			...
			if(ep.path.indexOf("like")>=0){
				...
				// UPV ADDED Check if the user has already make a like in that point/POI. He /she can only like once (for Fallas)
				
				var q = {coords: input.coords,user:input.user} ;
		
				Like.find(q).exec(function(e,res){
					if (e){
						console.log("there was an error");
						cb(500,{});
						return;
			...
			if(ep.path.indexOf("rating")>=0){
				...
				Rating.find(q).exec(function(e,res){
					if (e){
						console.log("there was an error");
						cb(500,{});
						return;
...
var getPoisByLocationAndCity = function(lon, lat, cityid, cb, options){
	...
	if (options.tags){
		q.$where = 'this.tags.indexOf("'+options.tags+'") != -1'; //UPV ADDED
	}
	Poi.find(q)
		.or([{"public": true}, {"user._user": options.uid}])
		//.and({$where : 'this.tags.indexOf("'+options.tags+'") != -1'} )		//ADDED BY UPV
		.skip(options.offset)
